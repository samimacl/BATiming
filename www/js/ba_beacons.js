...
var ba_beacons = (function () {
    ...
    ba_beacons.delegate = null;
    ba_beacons.beaconsInRegion = [];

    // some flags 
    ba_beacons.beaconsInUse = true;
    ba_beacons.inBeaconRegion = false;
    ba_beacons.checkAppStartInRegion = true;
    ba_beacons.allowBeaconInfos = false;

    // determin if to stop ranging after beacon was found in reagion until re-entry in region
    // give user choice in settings
    ba_beacons.stopRangingBeaconsUntilNewEntry = true;

    // will be set when beacon found, not part of beaconRegion!
    ba_beacons.beaconMinor = 'unbekannt';

    ba_beacons.scheduledExitFunc = "";


    ba_beacons.initialize = function () {
        // this is done after device ready, not here
        ba_beacons.initBeaconDelegates();
        ba_beacons.allowBeaconInfos = true;
    };
    ...
    ba_beacons.stopBeaconUsage = function () {
        ba_beacons.delegate = null;
        ba_beacons.allowBeaconInfos = false;
        ba_beacons.beaconMinor = 'unbekannt';
    };
     ...
     */
    ba_beacons.initBeaconDelegates = function () {

        // var delegate = new cordova.plugins.locationManager.Delegate();

        // save delegate to have access and delete again later, if user switches off beacons
        ba_beacons.delegate = new cordova.plugins.locationManager.Delegate();

        if (ba_beacons.delegate) {

            ba_beacons.delegate.didExitRegion = function (pluginResult) {
                ...
                // ba_beacons.scheduledExitFunc = setTimeout(function() { ba_beacons.exitRegionFunc(); }, 3000);
                ba_beacons.exitRegionFunc(beaconRegion);

            };

            ba_beacons.delegate.didEnterRegion = function (pluginResult) {

                // check, if exit is scheduled (happend only 10 sec ago)
                //if (ba_beacons.scheduledExitFunc) { 
                //    clearTimeout(ba_beacons.scheduledExitFunc); 
                //} else {
                console.log("didEnterRegion...ENTER DONE" + "<br>" + JSON.stringify(pluginResult));
                ba_beacons.enterRegionFunc(beaconRegion);
                //};
            };

            ba_beacons.delegate.didDetermineStateForRegion = function (pluginResult) {
                ...
                if (pluginResult.state == "CLRegionStateInside") {
                    ...
                    if (ba_beacons.allowBeaconInfos) {
            ...
            ba_beacons.delegate.didStartMonitoringForRegion = function (pluginResult) {
                console.log('didStartMonitoringForRegion:', + '<br>' + JSON.stringify(pluginResult));
            };


            ba_beacons.delegate.didRangeBeaconsInRegion = function (data) {
                ...
                if (data.beacons[0]) {
                    ...
                    ba_beacons.beaconsInRegion.push(data.beacons[index].minor);
                    ba_beacons.beaconMinor = data.beacons[index].minor;
                    ba_beacons.inBeaconRegion = true;

                    if (ba_beacons.stopRangingBeaconsUntilNewEntry) {
                        locationManager.stopRangingBeaconsInRegion(beaconRegion)
                ...
                console.log('Beacons-Array: ' + ba_beacons.beaconsInRegion);
                wf_app.manageZoneStates();
                wf_app.manageButtonStates();
            ...
            locationManager.setDelegate(ba_beacons.delegate);

            // Start monitoring 
            // To save battery: Start rainging only when region was entered.

            var region = ba_beacons.beaconRegionLoctigo;
            ...
            if (ba_beacons.checkAppStartInRegion) {
    ...
    ba_beacons.exitRegionFunc = function (beaconRegion) {
    ...
    ba_beacons.enterRegionFunc = function (beaconRegion) {
